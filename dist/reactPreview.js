...
var _propTypesMarkdown = require('./propTypesMarkdown');

var _propTypesMarkdown2 = _interopRequireDefault(_propTypesMarkdown);
...
function generatePreview(markdown, primary, context) {
  ...
  markdown = markdown.replace('#PROPS#', (0, _propTypesMarkdown2.default)(primary));
  ...
  while (match = codeBlockRegex.exec(markdown)) {
    if (match[1] === 'preview') {
      ...
      var newCode = match[0].replace('```preview', '```');
